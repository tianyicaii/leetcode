...
you should divide [x, y, z] into two intervals: [x, (x + y) / 2, z] and [(x + y) / 2 + 1, y, n]
...
*/
    ...
    // partition [0-359] to n machines, add one machine at a time.
    public List<List<Integer>> consistentHashing (int n) {
        ...
        for (int i = 1; i < n; ++i) {  // add each subsequent machine
            List<Integer> newMachine = new ArrayList<Integer>();
            int max = 0;
            for (int j = 1; j < i; ++j) {  // from the second to [i-1] current existing intervals, find the largest one to split
                if (ans.get(j).get(1) - ans.get(j).get(0) + 1 >
                    ans.get(max).get(1) - ans.get(max).get(0) + 1)
                    max = j;
            }

            // split interval[max]
            int x = ans.get(max).get(0);
            int y = ans.get(max).get(1);
            ans.get(max).set(1, (x + y) / 2);  // reduce
