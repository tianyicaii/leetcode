...
// eliminate k/2 elements on each iteration (quick select)


int find_kth(
    std::vector<int>::iterator begin_a,
    std::vector<int>::iterator end_a,
    std::vector<int>::iterator begin_b,
    std::vector<int>::iterator end_b,
    int k) {
...
double findMedianSortedArrays(std::vector<int>& nums1, std::vector<int>& nums2) {
    ...
    if (len % 2 == 1) {
        ...
        return (
            find_kth(nums1.begin(), nums1.end(), nums2.begin(), nums2.end(), len / 2 - 1) +
            find_kth(nums1.begin(), nums1.end(), nums2.begin(), nums2.end(), len / 2)
        ) / 2.0;
    }
}
